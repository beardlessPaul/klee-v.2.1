...
cl::opt<bool> DebugCheckForImpliedValues(
    ...
    cl::opt<bool> Recursion("recursion", cl::init(false));
    cl::opt<bool> PointerConcretization("pointer-concr", cl::init(false));
    cl::opt<bool> Summarization("summarization", cl::init(false));
    cl::opt<bool> AllocateSymSize("allocate-sym-size", cl::init(false), cl::desc(""));
    cl::opt<uint32_t> Capacity("capacity", cl::desc(""), cl::init(1000u));
    cl::list<std::string> CharArrayAsString("char-str");
    cl::opt<bool> All("all", cl::init(false));
    ...
    if (OnlyReplaySeeds) {
      for (unsigned i = 0; i < N; ++i) {
        if (result[i] && !seedMap.count(result[i])) {
          terminateStateEarly(*result[i], "Unseeded path during replay",
...
Executor::StatePair Executor::fork(ExecutionState &current, ref<Expr> condition,
  ...
  if (res == Solver::True) {
    ...
    if (Summarization || All)
      sum.fork((*trueState).id, (*falseState).id, (*trueState).stack);
...
void Executor::addConstraint(ExecutionState &state, ref<Expr> condition) {
  ...
  if (Summarization || All)
  {
    std::vector<FunctionSummaries*>func_sum = sum.searchIntersectionFunction(state.stack);
...
void Executor::executeCall(ExecutionState &state, KInstruction *ki, Function *f,
  ...
  if (f && f->isDeclaration()) {
    ...
    {
      if (state.recursive_calls == 0)
        for(int recCalls = state.stack.size() - 1; recCalls > 0; recCalls--)
        {
          if(state.stack[recCalls].kf == kf)
          {
    ...
    if (Summarization)
    {
      ...
      else 
      {
        if (sumKf->completed)
        {
          ...
          addedStates.insert(addedStates.end(), sumAddedStates.begin(), sumAddedStates.end());
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::Ret: {
     ...
     if (Summarization || All) 
    {
      ...
      if ((func_sum = sum.searchFunction(state.stack[state.stack.size() - 1].kf)) != NULL)
      {
...
void Executor::updateStates(ExecutionState *current) {
 ...
 if (Recursion || All) {
  if (addedStates.size())
    addedStates[addedStates.size() - 1]->recursive_calls = current->recursive_calls;
 }
...
void Executor::run(ExecutionState &initialState) {
  ...
  while (!states.empty() && !haltExecution) {
    ...
    KInstruction *ki = state.pc;
    stepInstruction(state);
 if (PointerConcretization || All) 
 {
    for (auto &it : state.symbolics) {
      if (it.second->isPointer())
        continue;
      llvm::Value *value = const_cast<llvm::Value *>(it.first->allocSite);
      ...
      if (type->getTypeID() == llvm::Type::PointerTyID) {
        ...
        if (type->getTypeID() == llvm::Type::IntegerTyID) {
          it.second->setPointer();
          it.second->setPointerSize(type->getScalarSizeInBits() / 8);
        }
      }
    }
  }
...
void Executor::terminateStateEarly(ExecutionState &state, const Twine &message,
  ...
  if ((terminationType <= StateTerminationType::EXECERR &&
    ...
    interpreterHandler->processTestCase(
        state, (message + "\n").str().c_str(),
        terminationTypeFileExtension(terminationType).c_str(), nullptr);
  }
...
void Executor::executeAlloc(ExecutionState &state, ref<Expr> size, bool isLocal,
  ...
  if (CE || AllocateSymSize || All) {
    ...
    MemoryObject *mo =
        memory->allocate(/*CE->getZExtValue()*/capacity, isLocal, /*isGlobal=*/false,
...
void Executor::executeMakeSymbolic(ExecutionState &state,
  ...
  if (!replayKTest) {
    ...
    for (auto &s : CharArrayAsString)
    {
      if (s == array->name)
      {
        UpdateList ul(array, 0);
        ref<Expr> arrayAllocRead = ConstantExpr::alloc(array->size - 1, array->getDomain());
        ref<Expr> arrayAllocEq = ConstantExpr::alloc(0, array->getRange());
        ref<Expr> tempRead = ReadExpr::create(ul, arrayAllocRead);
        ref<Expr> constraint = EqExpr::create(tempRead, arrayAllocEq);
...
bool Executor::getSymbolicSolution(
  ...
  bool success = solver->getInitialValues(extendedConstraints, objects, values,
                                          state.queryMetaData, PointerConcretization || All);
  solver->setTimeout(time::Span());
  if (!success) {
